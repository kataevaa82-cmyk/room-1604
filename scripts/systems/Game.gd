...
func ensure_loading_overlay() -> void:
	...
	caption.text = Loc.t("КОМНАТА 1604  ·  ШЕСТНАДЦАТЫЙ ЭТАЖ")
...
# Язык, выбранный игроком руками. Пустая строка означает «решает площадка» —
# именно это и есть автоопределение по требованию 2.14. Настройка устройства,
# а не прогресс: из облака она не приезжает, как и звук.
var language_override := ""
...
func set_language_override(code: String) -> void:
...
func save_progress() -> void:
	...
	config.set_value("progress", "language", language_override)
...
func load_progress() -> void:
	...
	# Язык читается отдельно от прогресса: это настройка устройства, и в
	# облако она не уезжает — как и звук.
	language_override = str(config.get_value("progress", "language", ""))
