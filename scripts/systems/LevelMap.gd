...
func centered_text(font: Font, center: Vector2, text: String, font_size: int, color: Color) -> void:
	...
	draw_string(font, center + Vector2(-text_size.x * .5, text_size.y * .35), text,
...
func _draw() -> void:
	...
	if has_player:
		...
		draw_string(ThemeDB.fallback_font, here + Vector2(11, 5), "ВЫ",
			HORIZONTAL_ALIGNMENT_LEFT, -1, 12, PLAYER)

	draw_string(ThemeDB.fallback_font, Vector2(0, size.y - 8),
		"свет  ·  голос  ·  отражение  ·  время  ·  память  ·  порог",
