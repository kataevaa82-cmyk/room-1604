...
# Единственная дверь для всех бегущих сообщений игры — сюда сходятся полторы
# сотни вызовов из девяти кругов. Поэтому перевод стоит здесь, а не в них.
func show_message(text: String, duration := 2.8) -> void:
	if message_tween and message_tween.is_valid():
		message_tween.kill()
	message.text = Loc.t(text)
