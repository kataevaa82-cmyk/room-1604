...
func refresh() -> void:
	for i in range(CAPACITY):
		...
		# Название вещи приходит из таблиц круга, то есть по-русски. Слот —
		# единственное место, где оно попадает игроку на глаза, здесь и переводим.
		label.text = "%d  %s" % [i + 1, Loc.t(str(items[i]["name"]))]
