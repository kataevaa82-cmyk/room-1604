...
const FROST := {
	...
	"frost_glass": [Vector3(3.90, .80, 1.70), 0.0, "Лёд в стакане",
	...
	"frost_doorknob": [Vector3(-1.20, .90, -1.32), 0.0, "Ледяная дверная ручка",
...
func build_props() -> void:
	...
	frost_nodes["frost_glass"] = root.placed_model(self, "FrostGlass",
		load("res://assets/models/c2_toothbrush_glass.glb"), Vector3(3.90, .76, 1.70), Vector3.ZERO,
	...
	frost_nodes["frost_doorknob"] = root.placed_model(self, "FrostDoorknob",
		load("res://assets/models/corridor_door_hanger.glb"), Vector3(-1.20, .90, -1.32), Vector3(0, PI / 2, 0),
