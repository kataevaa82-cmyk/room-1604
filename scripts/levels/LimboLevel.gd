...
func run_audit() -> void:
	...
	if not require(interactor.entry("towels")["text"].contains("разворошил"),
	...
	hints.set_enabled(false)
	request_hint()
	if not require(hud.message.text == current_goal(),
	...
	use_tv()
	use_tv()
	if not require(tv_channel == 2 and tv_text.text.contains("КОРИДОР"), "tv service channel missing"): return
	use_bed_curtain()
