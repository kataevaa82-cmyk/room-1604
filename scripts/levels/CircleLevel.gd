...
# Предметы круга обязаны стоять по месту, а не висеть в воздухе и не тонуть в
# мебели. Проверка родилась из живой жалобы: в семи кругах из девяти сюжетные
# вещи держались на координатах, взятых на глаз, — счёт висел в метре над полом,
# коробка была утоплена в матрас по середину, лампа провалилась сквозь пол.
#
# Два условия, и оба обязательны:
#   * КАСАНИЕ — габариты предмета подходят к какой-нибудь геометрии номера ближе
#     чем на PROP_TOUCH. Так проходят и лежащие на мебели, и висящие на стене, и
#     приклеенные к стеклу: у всех есть, к чему прислониться.
#   * НЕ ВНУТРИ — центр предмета не находится внутри коллизии. Одного касания
#     мало: утопленная в кровать коробка касается кровати идеально.
#
# Мебель без коллизии (тумбочки, кресла, раковина) ловится первым условием по
# видимым габаритам, поэтому расхождение коллизии и меша больше не мешает.
const PROP_TOUCH := .12

func prop_report(node: Node3D, room_meshes: Array) -> Dictionary:
	var meshes: Array = []
	gather_meshes(node, meshes, null)
	if meshes.is_empty():
		return {}
	var box: AABB = mesh_bounds(meshes)
	var centre := box.position + box.size * .5
	var nearest := 99.0
	var nearest_name := ""
	for mesh in room_meshes:
		var instance := mesh as MeshInstance3D
		var other: AABB = instance.global_transform * instance.get_aabb()
		var distance := aabb_distance(box, other)
		if distance < nearest:
			nearest = distance
	...
	var inside := get_world_3d().direct_space_state.intersect_point(point, 1)
	return {
		"box": box,
		"touch": nearest,
		"touch_name": nearest_name,
		"inside": "" if inside.is_empty() else str((inside[0]["collider"] as Node).name)
	}

func aabb_distance(a: AABB, b: AABB) -> float:
	...
	var dz := maxf(maxf(b.position.z - a.end.z, a.position.z - b.end.z), 0.0)
	return Vector3(dx, dy, dz).length()

func check_props_placed(exempt: Array = []) -> bool:
	var room_meshes: Array = []
	gather_meshes(get_parent(), room_meshes, self)
	var failures: Array[String] = []
	for child in get_children():
		if not child is Node3D or not (child as Node3D).visible:
			continue
		if str(child.name) in exempt:
			continue
		var report := prop_report(child as Node3D, room_meshes)
		if report.is_empty():
			continue
		var box: AABB = report["box"]
		var where := "x[%.2f..%.2f] y[%.2f..%.2f] z[%.2f..%.2f]" % [
			box.position.x, box.end.x, box.position.y, box.end.y, box.position.z, box.end.z]
		if not str(report["inside"]).is_empty():
			failures.append("%s is buried inside %s %s" % [child.name, report["inside"], where])
		elif float(report["touch"]) > PROP_TOUCH:
			failures.append("%s hangs %.2f m away from anything (nearest: %s) %s" % [
				child.name, report["touch"], report["touch_name"], where])
	return require(failures.is_empty(),
		"props placed off the room (%d):\n  %s" % [failures.size(), "\n  ".join(failures)])

func gather_meshes(node: Node, out: Array, skip: Node) -> void:
	if node == skip:
		return
	if node is MeshInstance3D:
		out.append(node)
	for child in node.get_children():
		gather_meshes(child, out, skip)

func mesh_bounds(meshes: Array) -> AABB:
	var box := AABB()
	var first := true
	for mesh in meshes:
		var instance := mesh as MeshInstance3D
