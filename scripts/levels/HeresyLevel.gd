...
# точка обзора].
const SEALS := {
	"seal_box": [Vector3(1.90, .70, -1.45), 0.0, "Заклеенная коробка",
	...
	"seal_curtain": [Vector3(.80, 1.10, 5.40), 0.0, "Зашитый подол шторы",
		...
		Vector3(.80, 1.20, 4.95)],
	"seal_book": [Vector3(3.90, .76, 1.60), .30, "Склеенные страницы книги",
...
func build_props() -> void:
	...
	seal_nodes["seal_box"] = root.placed_model(self, "SealBox",
		load("res://assets/models/hotel_amenities.glb"), Vector3(1.90, .64, -1.45), Vector3.ZERO,
	...
	seal_nodes["seal_curtain"] = root.placed_model(self, "SealCurtain",
		load("res://assets/models/c2_dress_hanger.glb"), Vector3(.80, 1.86, 5.40), Vector3(0, PI / 2, 0),
		{"brass": "brass", "fabric": "linen"})
	seal_nodes["seal_book"] = root.placed_model(self, "SealBook",
		load("res://assets/models/books.glb"), Vector3(3.90, .76, 1.60), Vector3(.25, .10, .30),
...
func run_audit() -> void:
	...
	if not check_actions_bound(SEALS.keys()): return
