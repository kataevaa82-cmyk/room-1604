...
func build_props() -> void:
	...
	suitcase_prop = root.placed_model(self, "C2Suitcase", load("res://assets/models/c2_suitcase.glb"),
		Vector3(1.95, 0, -.55), Vector3(0, .28, 0),
	...
	dress_prop = root.placed_model(self, "C2Dress", load("res://assets/models/c2_dress_hanger.glb"),
		Vector3(-.70, 1.86, -.15), Vector3(0, PI / 2, 0), {"brass": "brass", "fabric": "flower_dark"})
	...
	catch_prop = root.placed_model(self, "C2Catch", load("res://assets/models/c2_window_catch.glb"),
		Vector3(1.62, .01, -.72), Vector3(0, -.6, 0), {"brass": "brass", "handle": "chrome"})
...
func register_targets() -> void:
	...
	interactor.register("transom", Vector3(-2.65, 2.24, -2.86), Vector3(.80, .26, .30), {
		...
		"text": "Узкое окошко над входной дверью откинуто внутрь. Шпингалета на нём нет — сорван.",
		"usable": true})
	interactor.register("c2_suitcase", Vector3(1.95, .30, -.55), Vector3(.56, .52, .34), {
		"title": "Чужой чемодан",
	...
	interactor.register("c2_dress", Vector3(-.70, 1.50, -.15), Vector3(.30, .80, .46), {
...
func use_suitcase() -> void:
	...
	announce_pickup("catch", "Под чемоданом — латунный шпингалет.")
	reward("c2_suitcase")
...
func enter_hold() -> void:
	...
	interactor.set_text("transom",
		"Фрамуга закрыта твоими руками. Её можно открыть снова.")
...
func release_letters() -> void:
	...
	if inventory.selected() != "bundle":
		if inventory.has("bundle"):
			...
			hud.show_message("Фрамуга открыта. Ветер идёт, но отдать ему нечего.", 3.4)
			interactor.set_text("transom", "Фрамуга снова откинута. За ней тянет холодом.")
		return
	released = true
...
func focus_next_source() -> void:
	for id in SOURCE_IDS:
		...
		hints.set_focus(points[id], [
			...
			"Осталось закрыть: %s. Нажми E." % SOURCE_NAMES[id]])
		hints.reset_timer()
		return
	hints.clear_focus()
