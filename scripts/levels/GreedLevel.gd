...
const MONEY := {
	...
	"cash_robe": [Vector3(-3.60, 1.20, -.30), .30, "В кармане халата",
		"Три тысячи мелкими купюрами, свёрнутые в трубочку. Халат отельный, деньги — нет.",
		"debt_bar_tab", Vector3(-3.15, 1.25, -.30)],
	"cash_jacket": [Vector3(2.40, .90, 1.10), .20, "Во внутреннем кармане пиджака",
		"Восемьсот рублей мелкими купюрами. Карман застёгнут на всякий случай.",
		"debt_maid_tip", Vector3(1.95, 1.25, 1.10)],
	"cash_rug": [Vector3(1.80, .05, 3.60), 0.0, "Под ковриком у кресла",
		"Сто двадцать тысяч одной пачкой. Ковёр лежит ровно — его поднимали аккуратно.",
		"debt_neighbor_debt", Vector3(1.80, 1.15, 3.20)],
	"cash_floor": [Vector3(.20, .03, 1.20), 0.0, "В щели у плинтуса",
...
const DEBTS := {
	"debt_room_bill": [Vector3(3.90, .90, .75), 0.0, "Счёт за номер",
		"Пятнадцать тысяч, просрочено на неделю. Печать отеля смазана — его переставляли на стол много раз.",
		Vector3(3.40, 1.25, .75)],
	"debt_bar_tab": [Vector3(2.70, .85, 4.40), 0.0, "Счёт из бара",
		"Три тысячи, почерком от руки. Внизу подпись — не твоя, но похожа.",
		Vector3(2.70, 1.25, 4.00)],
	"debt_maid_tip": [Vector3(-2.70, 1.40, 1.60), 0.0, "Чаевые горничной",
		"Записка на подносе для полотенец: «Недодано восемьсот». Почерк ровный, без злости.",
		Vector3(-2.30, 1.40, 1.30)],
	"debt_neighbor_debt": [Vector3(3.70, .90, 3.60), 0.0, "Долг соседям по этажу",
		"Сто двадцать тысяч, дата двухмесячной давности. Число обведено дважды.",
		Vector3(3.30, 1.30, 3.60)],
	"debt_porter_loan": [Vector3(2.95, .60, -2.60), 0.0, "Занял у портье",
		"Сорок тысяч без расписки — на слово. Слово, судя по всему, не сдержали.",
		Vector3(2.50, 1.10, -2.30)]
}
