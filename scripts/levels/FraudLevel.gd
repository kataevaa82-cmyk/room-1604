...
const DECEITS := {
	...
	"fake_photo": [Vector3(1.35, 1.01, 5.40), 0.0, "Фотография в рамке",
		...
		Vector3(1.35, 1.20, 4.75)],
	"fake_key": [Vector3(2.60, .74, -2.70), 0.0, "Запасной ключ на тумбочке",
		...
		Vector3(2.50, 1.15, -2.20)],
	"fake_view": [Vector3(.80, 1.45, 5.18), 0.0, "Вид за окном",
...
func build_props() -> void:
	...
	deceit_nodes["fake_photo"] = root.placed_model(self, "FakePhoto",
		load("res://assets/models/fancy_picture_frame.glb"), Vector3(1.35, 1.01, 5.47), Vector3(0, PI, 0),
		{"frame": "brass"})
	deceit_nodes["fake_key"] = root.placed_model(self, "FakeKey",
		load("res://assets/models/corridor_door_hanger.glb"), Vector3(2.60, .74, -2.70), Vector3(0, .4, 0),
		{"brass": "brass", "tag": "flower_dark"})
	deceit_nodes["fake_view"] = root.placed_model(self, "FakeView",
		load("res://assets/models/window_sill.glb"), Vector3(.80, 1.45, 5.53), Vector3.ZERO,
