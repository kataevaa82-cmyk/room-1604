...
func run_flow_audit()->void:
	# В изолированной сборочной среде user:// находится вне рабочей папки и
	# намеренно недоступен на запись. Сам тест не должен зависеть от профиля
	# Windows: редактор пишет в build/, упакованная игра — рядом со своим EXE.
	var audit_directory := OS.get_executable_path().get_base_dir()
	if OS.has_feature("editor"):
		audit_directory = ProjectSettings.globalize_path("res://build")
	DirAccess.make_dir_recursive_absolute(audit_directory)
	Game.save_path = audit_directory.path_join("progress_flow_audit.cfg")
