...
func _ready() -> void:
	...
	# Меню и все его кнопки уже собраны, но HTML-лоадер пока перекрывает canvas и
	# не даёт игроку начать взаимодействие. Запрашиваем LoadingAPI.ready() сейчас:
	# Platform дождётся SDK и применит его язык, а затем одним шагом сообщит
	# готовность площадке и снимет блокирующий лоадер.
